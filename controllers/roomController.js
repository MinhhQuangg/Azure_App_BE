...
const createRoom = async (req, res) => {
  try {
    const { name, description, adminId, avatarColor, avatarText, lastMessage } =
      req.body;
    ...
    if (!avatarColor || !avatarText) {
      return res
        .status(400)
        .json({ error: 'Chat room avatar color and text is required.' });
    }
...
const updateRoomData = async (req, res) => {
  try {
    ...
    return res.json({
      message: "Chat room's description updated successfully.",
      chatroom,
    });
  } catch (err) {
...
const handleMemberRequest = async (req, res) => {
  try {
    ...
    if (!checkAdmin(chatId, userId)) {
      return res
        .status(400)
        .json({ message: 'Only admin can approve member.' });
    }
...
const getReadStatus = async (req, res) => {
  try {
    ...
    return res.json({
      message: 'Chat room read status updated successfully',
...
const loadChatRooms = async (req, res) => {
  try {
    ...
    const nextCursor =
      chatRooms.length === limit ? chatRooms[chatRooms.length - 1].id : null;
...
const getMembers = async (req, res) => {
  try {
    ...
    return res.json({
      message: 'Get chat room members successfully.',
      members,
    });
  } catch (err) {
    console.error('Error loading chat rooms:', err.message);
...
const getPendingMembers = async (req, res) => {
  try {
    ...
    return res.json({
      message: 'Get chat room pending members successfully.',
      members,
    });
  } catch (err) {
    console.error('Error loading chat rooms:', err.message);
...
const getPendingRooms = async (req, res) => {
  ...
  try {
    ...
    const sortedRooms = pendingRooms
      ...
      .sort(
        (a, b) =>
          new Date(b.chatRoom.updated_at) - new Date(a.chatRoom.updated_at)
      );
